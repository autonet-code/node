...
- Deploy Firestore rules for `substrate_viz` (world-readable,
  ...
  (`scripts/indexer/` is gitignored ops, living outside the repo).
...
- **The v3 sybil trade: MITIGATION BUILT 2026-07-08 (balance-weighted
  ...
  (`balanceOfAt`/`atnTotalSupplyAt`, Trace208, no delegation layer, so
  ...
  harm still has ONLY vetting as its dedicated defense, so vet params
...
- ~~world_persistence artifact_digest gap~~: FIXED 2026-07-08,
  ...
  changes, so it rides the open flag-day window.
...
6. CLAUDE.md is gitignored, so the machine-onboarding contract doesn't
...
- **ATN = money, reputation = voice (2026-07-08 evening): the
  ...
  1:1 with ATN, non-transferable), keeping the same household collapse, same
...
- **Owner binding is UNWIRED in the live path (2026-07-08):** the daemon
  ...
  surface (rotateOwner), which is also the key-loss recovery UI.
...
- MCP server quirk: any Claude Code session with the atn MCP
  configured silently stands up a full runtime on :7700; it should
