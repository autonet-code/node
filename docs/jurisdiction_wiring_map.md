...
3. `configureAndFinalize(...)` (`:132`) is **the attach + handoff step**:
...
Not a clone of Project, but a standalone router:
...
commons; see `venture_vault_design.md`). Any port must keep revenue custody in the
...
- **Direction decisions** (which way the jurisdiction goes, allocating the
...
- **Quality decisions** (what is good work, judging the produced): substrate
...
| Two-token split (transferable value + soulbound gov) for boats | `ARCHITECTURE_NOTES.md:9` | Reversed; per `BUILD_STATE.md:28`, ATNB is the single transferable gov token. |
...
1. **Charter anchor placement.** On Substrate.sol (introduces the first governed
   ...
   Report leans Registry; the user decides, since this is "constitutional prompt
   wording" territory.
