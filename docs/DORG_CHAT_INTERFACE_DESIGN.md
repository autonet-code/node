...
product" is — could be the shared governance/control interface, or could be
...
(`send_agent_message` — handles mid-session injection vs inbox automatically).
...
2. **Open-ended conversational registration (designed, not built):** instead of
   ...
   set. Key realization: there's not much diversity in endpoints — it's
...
3. **Member-run daemons + on-chain + jurisdiction (directional, endgame):**
   ...
   already supports this — `register_agent_on_chain`), and **join the dОrg
