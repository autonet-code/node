...
(msg.sender, amount) pairs that prove into it. These tests pin:

  - python tree mechanics (roots, proofs, exclusions, determinism),
  - the contract accepting exactly the ratified amounts,
  - the contract rejecting inflated amounts, foreign proofs, and
    claims against rootless (legacy-shaped) anchors,
...
def chain():
    ...
    tx = contract_f.constructor(deployer).transact({"from": deployer, "gas": 8_000_000})
